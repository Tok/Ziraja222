...
public class AppController implements Presenter, ValueChangeHandler<String> {
	...
	private void bind() {
		...
		eventBus.addHandler(UpdateLetterEvent.TYPE,
				new UpdateLetterEventHandler() {
					public void onUpdated(UpdateLetterEvent event) {
						questionView.doUpdateLetter();
					}
				});
		
		eventBus.addHandler(UpdateTotalEvent.TYPE,
				new UpdateTotalEventHandler() {
					public void onUpdated(UpdateTotalEvent event) {
						questionView.doUpdateTotal();
					}
				});
		
		eventBus.addHandler(UpdateAnswerEvent.TYPE,
				new UpdateAnswerEventHandler() {
					public void onUpdated(UpdateAnswerEvent event) {
						questionView.doUpdateAnswer();
					}
				});
		
		eventBus.addHandler(DoAnswerEvaluationEvent.TYPE,
				new DoAnswerEvaluationEventHandler() {
					public void onEvaluate(DoAnswerEvaluationEvent event) {
